...
onMount(() => {
	...
	darkModePreference.addEventListener("change", changeThemeWhenSchemeChanged, { passive: true });
	return () => {
...
<div class="relative z-50" role="menu" tabindex="-1" onmouseleave={hidePanel}>
    <button aria-label="Light/Dark Mode" role="menuitem" class="relative btn-plain scale-animation rounded-lg h-11 w-11 active:scale-90" id="scheme-switch" onclick={toggleScheme} onmouseenter={showPanel}>
        <div class="absolute transition-opacity" class:opacity-0={mode !== LIGHT_MODE} style="will-change: opacity;">
            <Icon icon="material-symbols:wb-sunny-outline-rounded" class="text-[1.25rem]"></Icon>
        </div>
        <div class="absolute transition-opacity" class:opacity-0={mode !== DARK_MODE} style="will-change: opacity;">
            <Icon icon="material-symbols:dark-mode-outline-rounded" class="text-[1.25rem]"></Icon>
        </div>
        <div class="absolute transition-opacity" class:opacity-0={mode !== AUTO_MODE} style="will-change: opacity;">
            <Icon icon="material-symbols:radio-button-partial-outline" class="text-[1.25rem]"></Icon>
        </div>
    </button>

    <div bind:this={panelRef} id="light-dark-panel" class="hidden lg:block absolute transition float-panel-closed top-11 -right-2 pt-5" style="will-change: transform, opacity;">
