...
## 개선 사항 (6차) - 대화 맥락 유지 기능 추가
### 문제점
- Langflow API가 stateless로 작동하여 이전 대화를 기억하지 못함
- session_id를 전송하지만 효과가 없음

### 해결 방안
1. **클라이언트 측 대화 히스토리 전송**
   - 최근 5개의 대화 내용을 서버로 전송
   - conversation_history 필드에 포함

2. **서버 측 프롬프트 개선**
   - enhancePromptWithSearchResults 함수에 대화 맥락 추가
   - 이전 대화를 프롬프트에 포함하여 LLM에 전달

### 구현 내용
- 클라이언트: messages 배열에서 최근 5개 메시지 추출
- 서버: conversation_history를 받아 프롬프트에 포함
- 대화 맥락이 있을 때 추가 지침 제공

## 개선 사항 (7차) - 대화 맥락 유지 개선
### 문제점 및 해결
1. **히스토리 범위 확대**
   - 기존: 서버에서 3개만 사용 → 6개로 확대
   - 클라이언트: 최대 8개 전송

2. **메시지 필터링 개선**
   - 현재 입력 중인 메시지 제외 (slice(0, -2))
   - 빈 메시지와 타이핑 중인 메시지 필터링
