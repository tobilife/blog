...
async function getPostsMetadata() {
	...
	try {
		...
		if (response.ok) {
			...
			cacheTimestamp = now;
			console.info("Posts metadata refreshed");
			return metadataCache;
		}
	} catch (error) {
...
export default async (request, context) => {
	...
	const isSocialCrawler =
		userAgent.toLowerCase().includes("kakaotalk") ||
	...
	const isNaverBot =
		userAgent
			.toLowerCase()
			.includes("yeti") || // Naver search bot
	...
	if (isSearchBot) {
		console.info("Search bot detected:", {
			url: request.url,
			userAgent: userAgent,
	...
	if (isSearchBot && request.method === "GET") {
		try {
			...
			try {
				...
				console.info(
					`Response time for ${isGooglebot ? "Googlebot" : "search bot"}: ${responseTime}ms, status: ${response.status}`,
				);
				...
				console.info(`${isGooglebot ? "Googlebot" : "Search bot"} got error response:`, response.status);
			} catch (timeoutError) {
				clearTimeout(timeoutId);
				console.info(`${isGooglebot ? "Googlebot" : "Search bot"} request timed out after ${Date.now() - startTime}ms`);
			}
			...
			const pageDescription =
				metadata?.description || "AI, RAG, Git/GitHub, 보험IT 등 다양한 개발 경험과 지식을 공유하는 기술 블로그";
			...
			const structuredData =
				isPostPage && metadata
					...
					: {
							...
							url: "https://tobilife.netlify.app/",
							description:
								"TobiLife(토비라이프) 블로그 - IT 개발자의 기술 블로그. AI, 보험IT 등 다양한 개발 경험과 지식을 공유합니다.",
							inLanguage: "ko-KR",
							author: {
								"@type": "Person",
								name: "TobiLife(토비라이프)",
			...
			const articleList =
				postsMetadata && postsMetadata.posts && !isPostPage
					? Object.entries(postsMetadata.posts)
...
<body>
    ...
    <footer>
        <p>© 2024 TobiLife. All rights reserved.</p>
			...
			console.info(`Generated fallback HTML for ${isGooglebot ? "Googlebot" : "search bot"} in ${responseTime}ms`);
