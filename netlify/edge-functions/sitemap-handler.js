...
export default async (request, context) => {
	...
	const isSitemapRequest =
		(url.pathname.includes("sitemap") && url.pathname.endsWith(".xml")) || url.pathname === "/sitemap.xml";
	...
	const isGoogleSearchConsole =
		userAgent.toLowerCase().includes("google") || userAgent.toLowerCase().includes("googlebot");
	...
	// XML인지 확인
	const isXML =
		body.trim().startsWith("<?xml") || body.trim().startsWith("<sitemap") || body.trim().startsWith("<urlset");
